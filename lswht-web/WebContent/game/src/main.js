...
{
	res: [
	{id:"begin", size:372, src:"images/1begin.png"},
	{id:"beginText", size:372, src:"images/begin_text.png"},
    {id:"end", size:372, src:"images/1end.png"},
    {id:"help", size:372, src:"images/2help.png"},
	{id:"btns", size:77, src:"images/btns.png"},
	{id:"startBtns", size:372, src:"images/start_btns.png"},
	...
	{id:"reward1", size:29, src:"images/reward1.png"},
	{id:"reward2", size:29, src:"images/reward2.png"},
	...
	{id:"myExchangeCodePage", size:29, src:"images/my-exchange-code.png"},
	{id:"huoDongGuiZePage", size:29, src:"images/huodongguize_page.png"}
...
{	
	if(this.begin == null)
	{
		...
		this.begin = begin;
		 //TODOBEGIN
		//开始按钮
		var playBtn = new Q.Button({id:"playBtn", image:this.getImage("startBtns")});
		playBtn.setUpState({rect:[0,0,412,62]});
		playBtn.setOverState({rect:[0,62,412,62]});
        playBtn.scaleX = sX;
        playBtn.scaleY = sY;
		...
		playBtn.y = this.height - (this.height*0.135);
		...
		//活动规则
		var guizeBtn = new Q.Button({id:"guizeBtn", image:this.getImage("beginText")});
		guizeBtn.setUpState({rect:[0,0,321,83]});
		guizeBtn.setOverState({rect:[0,0,321,83]});
        guizeBtn.scaleX = sX ;
        guizeBtn.scaleY = sY ;
		guizeBtn.x = 0;
		guizeBtn.y = this.height - (this.height*0.07);
		guizeBtn.onEvent = function(e)
        {
            Q.Button.prototype.onEvent.call(this, e);
            if(e.type == "mouseup" || e.type == "touchend")
            {
            	game.showHuoDongGuiZePage();
            }else if(e.type == "mouseout")
            {
                game.context.canvas.style.cursor = "";
            }
        }
		this.guizeBtn = guizeBtn;
		
		//我的兑换码
		var duihuanmaBtn = new Q.Button({id:"duihuanmaBtn", image:this.getImage("beginText")});
		duihuanmaBtn.setUpState({rect:[0,83,321,83]});
		duihuanmaBtn.setOverState({rect:[0,83,321,83]});
        duihuanmaBtn.scaleX = sX ;
        duihuanmaBtn.scaleY = sY ;
		duihuanmaBtn.x = duihuanmaBtn.width*sX;
		duihuanmaBtn.y = this.height - (this.height*0.07);
		duihuanmaBtn.onEvent = function(e)
        {
            Q.Button.prototype.onEvent.call(this, e);
            if(e.type == "mouseup" || e.type == "touchend")
            {
				game.showMyExchangeCodePage();
            }else if(e.type == "mouseout")
            {
                game.context.canvas.style.cursor = "";
            }
        }
		this.duihuanmaBtn = duihuanmaBtn;
	}
	
	this.state = STATE.MENU;
	this.stage.addChild(this.begin, this.playBtn, this.guizeBtn,this.duihuanmaBtn);
}
game.showHuoDongGuiZePage = function(){
	var me = this;
	
	var huoDongGuiZePage = new Q.Bitmap({id:"huoDongGuiZePage", image:this.getImage("huoDongGuiZePage")});
    var sY = huoDongGuiZePage.scaleY = this.stage.height/huoDongGuiZePage.height;
    var sX = huoDongGuiZePage.scaleX = this.stage.width/huoDongGuiZePage.width;
    huoDongGuiZePage.x = 0;
    huoDongGuiZePage.y = 0;
	
	this.huoDongGuiZePage = huoDongGuiZePage;
	
	//我的兑换码 返回
	var myHuoDongGuiZePageReturnBtn = new Q.Button({id:"myHuoDongGuiZePageReturnBtn", image:this.getImage("dhmbtn")});
	myHuoDongGuiZePageReturnBtn.setUpState({rect:[0,0,640,83]});
	myHuoDongGuiZePageReturnBtn.scaleX = sX ;
	myHuoDongGuiZePageReturnBtn.scaleY = sY ;
	myHuoDongGuiZePageReturnBtn.x = 0;
	myHuoDongGuiZePageReturnBtn.y = this.height - (this.height*0.07);

	myHuoDongGuiZePageReturnBtn.onEvent = function(e)
    {
        Q.Button.prototype.onEvent.call(this, e);
        if(e.type == "mouseup" || e.type == "touchend")
        {
	...
	this.myHuoDongGuiZePageReturnBtn = myHuoDongGuiZePageReturnBtn;
	
	this.stage.addChild(this.huoDongGuiZePage,this.myHuoDongGuiZePageReturnBtn);
	this.stage.step();
}
game.showMyExchangeCodePage = function(){
	var me = this;
	
	var myExchangeCodePage = new Q.Bitmap({id:"myExchangeCodePage", image:this.getImage("myExchangeCodePage")});
    var sY = myExchangeCodePage.scaleY = this.stage.height/myExchangeCodePage.height;
    var sX = myExchangeCodePage.scaleX = this.stage.width/myExchangeCodePage.width;
    myExchangeCodePage.x = 0;
    myExchangeCodePage.y = 0;
	
	this.myExchangeCodePage = myExchangeCodePage;
	
	
	//我的兑换码 返回
	var myExchangeCodePageReturnBtn = new Q.Button({id:"myExchangeCodePageReturnBtn", image:this.getImage("dhmbtn")});
	myExchangeCodePageReturnBtn.setUpState({rect:[0,0,640,83]});
	myExchangeCodePageReturnBtn.scaleX = sX ;
	myExchangeCodePageReturnBtn.scaleY = sY ;
	myExchangeCodePageReturnBtn.x = 0;
	myExchangeCodePageReturnBtn.y = this.height - (this.height*0.07);

	myExchangeCodePageReturnBtn.onEvent = function(e)
    {
        Q.Button.prototype.onEvent.call(this, e);
        if(e.type == "mouseup" || e.type == "touchend")
        {
	...
	this.myExchangeCodePageReturnBtn = myExchangeCodePageReturnBtn;
	
	this.stage.addChild(this.myExchangeCodePage,this.myExchangeCodePageReturnBtn);
	this.stage.step();
};
...
game.showMain = function()
{
	...
	if(this.island == null)
	{
		...
		monkey.scaleX = island.scaleX * 0.6;
		monkey.scaleY = island.scaleY * 0.6;
	...
	this.monkey.y = this.height - this.monkey.getCurrentHeight() - (0.0893*this.height);
	this.monkey.dirX = 0;
	this.monkey.dirY = 0;
...
game.createPeachs = function()
{
	...
	for(var i = 0; i < this.maxPeachs; i++)
	{
		...
		peach.scaleX = this.island.scaleX * 0.8;
		peach.scaleY = this.island.scaleY * 0.8;
...
game.checkCollision = function()
{
	...
	for(var i = 0; i < peachs.length; i++)
	{
		...
		/*
		if( dx >= 0 ){
			if( dx <= monkey.getCurrentWidth() && dy <= peach.getCurrentHeight() ){
				if( peach.currentScore < 0 ){
					//this.gameOver();
					//console.log(peach.currentScore);
					console.log('end');
					console.log(peach.currentScore);
				}
				console.log('out :' + peach.currentScore);
			...
			if( peach.x + peach.getCurrentWidth() >= monkey.x && dy <= peach.getCurrentHeight() ){
				if( peach.currentScore < 0 ){
					//this.gameOver();
					//console.log(peach.currentScore);
					console.log('end');
				...
				console.log('out1 :' + peach.currentScore);
				this.addScore(peach, peach.currentScore);
				peach.y += 1000;
				return true;
			}
...
game.showEnd = function(){
    ...
    //结束画面
	var me = this;
	var endPicIndex = Math.floor((Math.random()*100))%4 + 1;
	var endPic = 'reward' + endPicIndex;
	
    var end = new Q.Bitmap({id:"end", image:this.getImage(endPic)});
	...
	*/
	...
	if( endPicIndex == 4 ){
		var dhmbtn = new Q.Button({id:"dhmbtn", image:this.getImage("dhmbtn")});
	    dhmbtn.setUpState({rect:[0,0,324,79]});
	    dhmbtn.scaleX = sX;
	    dhmbtn.scaleY = sY;
	    dhmbtn.x = 31*sX;
	    dhmbtn.y = 1020*sY;
	    dhmbtn.onEvent = function(e)
	    {
	        Q.Button.prototype.onEvent.call(this, e);
	        if(e.type == "mouseup" || e.type == "touchend")
	        {
	            trace("get exchange code btn click");
	            game.showShareMask();
	    ...
	    this.stage.addChild(this.end,this.dhmbtn);
	}else{
		var dhmbtn = new Q.Button({id:"dhmbtn", image:this.getImage("dhmbtn")});
	    dhmbtn.setUpState({rect:[0,0,324,79]});
	    dhmbtn.scaleX = sX;
	    ...
	    var replayBtn = new Q.Button({id:"replayBtn", image:this.getImage("dhmbtn")});
	    replayBtn.setUpState({rect:[0,0,324,120]});
	    replayBtn.scaleX = sX;
	    replayBtn.scaleY = sY;
	    replayBtn.x = 31*sX;
	    replayBtn.y = 966*sY;
	    ...
	    this.stage.addChild(this.end,this.dhmbtn,this.replayBtn);
	}
...
game.showShareMask = function(){
	var shareMask = new Q.Bitmap({id:"shareMask", image:this.getImage("sharemask")});
    var sY = shareMask.scaleY = this.stage.height/shareMask.height;
    var sX = shareMask.scaleX = this.stage.width/shareMask.width;
    shareMask.x = 0;
    shareMask.y = 0;
...
game.showInputMobilePage = function(){
	/*
